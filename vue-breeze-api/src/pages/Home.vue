<template>
    <div class="container mt-3">
        <div class="grid grid-cols-4 gap-x-2 gap-y-5 mx-auto">
            <div class="col-span-4 row-span-1 lg:col-span-3 lg:row-span-3">
                <div class="flex relative">
                    <button @click="showDropdownSearch($event)" id="dropdown-button"
                        ...
                        type="button">
                        {{ visionTextSortBy }}
                        <svg aria-hidden="true" class="w-4 h-4 ml-1"
                    ...
                    <div id="dropdown-search"
                        ...
                        <ul class="text-sm text-gray-700 dark:text-gray-200"
                            ...
                            <li>
                                <button @click="changeSortBy('likes', searchPayload)
                                    " type="button"
                                    class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                    Lượt thích
                                </button>
                            ...
                            <li>
                                <button @click="changeSortBy('views', searchPayload)
                                    " type="button"
                                    class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                    Lượt xem
                                </button>
                    ...
                    <ul class="flex flex-wrap -mb-px" id="myTab"
                        ...
                        <li class="mr-2" role="presentation">
                            <button
                                ...
                                role="tab" aria-controls="tag"
                                aria-selected="false">Chủ
                                đề</button>
                        </li>
                    </ul>
                </div>
                <div id="myTabContent">
                            ...
                            <!-- <div class="flex items-center sort-data mb-2">
                                ...
                                <select id="countries"
                                    ...
                                    <option selected value="new"
                                        class="form-select-option py-2">Mới nhất
                                    </option>
                                    <option value="view"
                                        class="form-select-option py-2">Lượt xem
                                        nhiều nhất</option>
                                    <option value="like"
                                        class="form-select-option py-2">Đánh giá
                                        nhiều nhất</option>
                    ...
                    <div class="pt-2 rounded-lg dark:bg-gray-800" id="tag"
                            ...
                            <!-- <div class="flex items-center sort-data mb-2">
                                ...
                                <select id="countries"
                                    ...
                                    <option value="question"
                                        class="form-select-option py-2">Nhều câu hỏi
                                        nhất</option>
                                    <option value="post"
                                        class="form-select-option py-2">Nhiều bài
                                        viết nhất</option>
                                    <option value="follower"
                                        class="form-select-option py-2">Nhiều người
                                        theo dõi nhất</option>
                                </select>
                            </div> -->
                            <div
                        ...
                        <div v-else>
                            <h5 class="font-bold text-lg text-gray-500">Không có chủ
                                đề
                                nào.</h5>
                        </div>
                        <div
            ...
            <div class="col-span-4 p-3 box-popular_questions h-100 row-span-1 lg:col-span-1 lg:row-span-1"
                v-if="popularQuestions && popularQuestions.length != 0">
                <h5 class="text-center pb-3 font-bold text-blue-500">Câu hỏi phổ biến nhất</h5>
                <div v-for="item in popularQuestions" :key="item.id">
                    <QuestionSidebar :data="item" />
                </div>
            </div>
            <div class="col-span-4 p-3 box-popular_posts h-100 row-span-1 lg:col-span-1 lg:row-span-1"
                v-if="popularPosts && popularPosts.length != 0">
                <h5 class="text-center pb-3 font-bold text-blue-500">Bài viết phổ biến nhất</h5>
                <div v-for="item in popularPosts" :key="item.id">
...
const popularTags = ref([]), popularPosts = ref([]), popularQuestions = ref([])
...
const fetchData = () => {
    ...
    axios
        .get("/api/posts")
        .then((response) => {
            response.data.popular_posts.forEach((item) => {
                item.author.avatar = 'http://localhost:8000/images/' + item.author.avatar
            });
            popularPosts.value = response.data.popular_posts
            // console.log('popular', response.data.popular_posts);
        })
        .catch((error) => {
            console.log(error);
        });
...
onMounted(async () => {
    ...
    await searchStore.handleGetSearchAll()
    initFlowbite()
    fetchData()
    dataSearch.value = searchStore.getDataSearch
    pageLoaded(700)
...
.box-popular_questions, .box-popular_posts {
    box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;
    border-radius: 6px;
}
</style>
